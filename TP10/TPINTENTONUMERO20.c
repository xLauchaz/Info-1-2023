...
int main(int argc, char *argv[])
{
	...
	char *api_number = "http://numbersapi.com/random/trivia?json"; /*se cambio https://numbersapi.com/random/trivia por http://numbersapi.com/random/trivia?json
	la cual era necesaria para obtener el JSON de la pagina*/
				...
				if(chat_id == chat_id_previo)
				{
					...
					if (strstr(message_text, "Dato") != NULL)
					{
						...
						if (res == CURLE_OK)
						{
							...
							cJSON *number_json = cJSON_GetObjectItemCaseSensitive(json2, "text"); /*Se modifico la palabra number por text, ya que e
							ahi se encuentra el dato en el JSON que devuelve la pagina*/
							...
							// Enviar la respuesta de la API de frases de numeros a traves de Telegram
							...
							if (curl)
							{
								char message[2048] = {0};
								//printf("%s\n",number);
								snprintf(message, sizeof(message), "%schat_id=%ld&text=%s%s", telegram_api_url, chat_id, "ESTA ES LA FRASE DEL DIA: ", number);/*se elimino
								el ultimo %s y el valor url, ya que retornaba null, tambien se quitaron los 2 \n que se encontraban despues de ESTA ES LA FRASE DEL DIA, se agregó [ : ] para hacer mas amigable el texto*/
